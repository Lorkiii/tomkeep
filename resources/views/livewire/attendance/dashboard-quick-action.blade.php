...
<div class="fixed bottom-5 right-5 z-40 sm:bottom-8 sm:right-8 lg:bottom-10 lg:right-10" x-data>
    ...
    @else
        {{--
            ...
            We use a browser confirm() here because it is simple, readable,
            and matches the user's requirement to prevent accidental clicks.
        --}}
        <button
            type="button"
            ...
            x-on:click="if (confirm(@js($state['confirmText'] ?? 'Confirm attendance action?'))) { $wire.mark() }"
        ...
        >
            ...
            <span wire:loading.remove wire:target="mark">
            ...
            <svg wire:loading wire:target="mark" class="h-7 w-7 animate-spin text-current" viewBox="0 0 24 24" fill="none" aria-hidden="true">

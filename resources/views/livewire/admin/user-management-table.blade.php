                ...
                <article wire:key="managed-user-card-{{ $user->id }}" class="rounded-[1.5rem] border border-slate-200 bg-[#fbfbfc] p-4 shadow-[0_20px_45px_-35px_rgba(15,23,42,0.35)]">
                    <div class="flex flex-col gap-4">
                        ...
                        <div class="grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                            <p><span class="font-semibold text-slate-500">Last Seen:</span> {{ optional($user->last_seen_at)->format('M j, Y g:i A') ?: 'Not recorded' }}</p>
                            <p><span class="font-semibold text-slate-500">Student Code:</span> {{ $user->student_code ?: 'Pending generation' }}</p>
                        </div>

                        <div class="flex flex-col gap-2 sm:flex-row sm:justify-end">
                            <a href="{{ route('admin.users.show', $user) }}" class="inline-flex items-center justify-center rounded-full border border-[#d5e0f0] bg-white px-4 py-2 text-sm font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3]">View</a>
                            <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center justify-center rounded-full border border-[#d5e0f0] bg-[#f7f9fc] px-4 py-2 text-sm font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3] hover:bg-white">Edit</a>
                        </div>
                    </div>
                </article>
            @empty
                    ...
                    <tbody class="divide-y divide-slate-200 text-slate-600">
                        @forelse($users as $user)
                            ...
                            <tr wire:key="managed-user-{{ $user->id }}">
                                ...
                                <td class="px-4 py-4 align-top text-slate-500">{{ optional($user->last_seen_at)->format('M j, Y g:i A') ?: 'Not recorded' }}</td>
                                <td class="px-4 py-4 align-top text-right">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        <a href="{{ route('admin.users.show', $user) }}" class="inline-flex rounded-full border border-[#d5e0f0] bg-white px-3 py-2 text-xs font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3]">View</a>
                                        <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex rounded-full border border-[#d5e0f0] bg-[#f7f9fc] px-3 py-2 text-xs font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3] hover:bg-white">Edit</a>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-14 text-center text-sm text-slate-500">No users matched the current filters.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

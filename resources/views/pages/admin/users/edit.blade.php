    ...
    <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <h1 class="mt-2 text-2xl font-black tracking-[0.08em] text-[#1e4fa3] sm:text-3xl">Edit
                {{ $managedUserName !== '' ? $managedUserName : 'User Profile' }}</h1>
            <p class="mt-3 max-w-3xl text-sm text-slate-600">Update profile fields, role assignment, and account
                metadata from one form.</p>
        </div>

        <a href="{{ route('admin.users.show', $managedUser) }}"
            class="inline-flex items-center justify-center rounded-full border border-[#d5e0f0] bg-white px-4 py-2 text-sm font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3] sm:w-fit">Cancel</a>
    </div>
    ...
    <form action="{{ route('admin.users.update', $managedUser) }}" method="POST"
        ...
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            <div><label for="managed-user-username"
                    class="block text-sm font-semibold text-slate-600">Username</label><input id="managed-user-username"
                    name="username" type="text" value="{{ old('username', $managedUser->username) }}"
            ...
            <div><label for="managed-user-email" class="block text-sm font-semibold text-slate-600">Email</label><input
                    id="managed-user-email" name="email" type="email" value="{{ old('email', $managedUser->email) }}"
                    ...
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-role" class="block text-sm font-semibold text-slate-600">Role</label><select
                ...
                </select>@error('role')<p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-first-name" class="block text-sm font-semibold text-slate-600">First
                    ...
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-middle-name" class="block text-sm font-semibold text-slate-600">Middle
            ...
            <div><label for="managed-user-gender"
                    class="block text-sm font-semibold text-slate-600">Gender</label><select id="managed-user-gender"
                    name="gender"
                    class="mt-2 w-full rounded-[1.25rem] border border-[#d5e0f0] bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-[#1e4fa3]">
                    <option value="">Select</option>@foreach(['Male', 'Female', 'Other'] as $gender)<option
                        value="{{ $gender }}" @selected(old('gender', $managedUser->gender) === $gender)>{{ $gender }}
                    </option>@endforeach
                </select>@error('gender')<p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-date-of-birth" class="block text-sm font-semibold text-slate-600">Date of
                    Birth</label><input id="managed-user-date-of-birth" name="date_of_birth" type="date"
                    ...
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-contact-number" class="block text-sm font-semibold text-slate-600">Contact
                    Number</label><input id="managed-user-contact-number" name="contact_number" type="text"
                    value="{{ old('contact_number', $managedUser->contact_number) }}"
                    ...
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-school-attended" class="block text-sm font-semibold text-slate-600">School
                    Attended</label><input id="managed-user-school-attended" name="school_attended" type="text"
                    value="{{ old('school_attended', $managedUser->school_attended) }}"
            ...
            <div><label for="managed-user-course"
                    class="block text-sm font-semibold text-slate-600">Course</label><input id="managed-user-course"
                    name="course" type="text" value="{{ old('course', $managedUser->course) }}"
                    ...
                    <p class="mt-2 text-sm text-rose-600">{{ $message }}</p>@enderror</div>
            <div><label for="managed-user-number-of-hours" class="block text-sm font-semibold text-slate-600">Required
                    Hours</label><input id="managed-user-number-of-hours" name="number_of_hours" type="number" min="0"
            ...
            <div><label for="managed-user-province"
                    class="block text-sm font-semibold text-slate-600">Province</label><input id="managed-user-province"
                    name="province" type="text" value="{{ old('province', $address['province'] ?? '') }}"
            ...
            <div><label for="managed-user-barangay"
                    class="block text-sm font-semibold text-slate-600">Barangay</label><input id="managed-user-barangay"
                    name="barangay" type="text" value="{{ old('barangay', $address['barangay'] ?? '') }}"
            ...
            <div class="xl:col-span-2"><label for="managed-user-street-house-number"
                    class="block text-sm font-semibold text-slate-600">Street / House Number</label><input
                    id="managed-user-street-house-number" name="street_house_number" type="text"
        ...
        <div class="mt-6 grid gap-4 md:grid-cols-2">
            <label
                class="flex items-center gap-3 rounded-[1.25rem] border border-slate-200 bg-[#fbfbfc] px-4 py-4 text-sm font-semibold text-slate-700"><input
                    type="hidden" name="profile_completed" value="0"><input type="checkbox" name="profile_completed"
                    value="1" @checked(old('profile_completed', $managedUser->profile_completed))
                    class="h-4 w-4 rounded border-slate-300 text-[#1e4fa3] focus:ring-[#1e4fa3]">Profile
                completed</label>
            <label
                class="flex items-center gap-3 rounded-[1.25rem] border border-slate-200 bg-[#fbfbfc] px-4 py-4 text-sm font-semibold text-slate-700"><input
                    type="hidden" name="is_active" value="0"><input type="checkbox" name="is_active" value="1"
                    @checked(old('is_active', $managedUser->is_active))
                    class="h-4 w-4 rounded border-slate-300 text-[#1e4fa3] focus:ring-[#1e4fa3]">Mark attendance state
                as active</label>
        </div>

        <div class="mt-6 flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:flex-wrap sm:justify-end">
            <a href="{{ route('admin.users.show', $managedUser) }}"
                class="inline-flex items-center justify-center rounded-full border border-[#d5e0f0] bg-white px-4 py-2 text-sm font-semibold text-[#1e4fa3] transition hover:border-[#1e4fa3]">Cancel</a>
            <button type="submit"
                class="inline-flex items-center justify-center rounded-full border border-[#1e4fa3] bg-[#1e4fa3] px-5 py-2 text-sm font-semibold text-white transition hover:bg-[#173d79]">Save
                User Changes</button>
        </div>
    </form>
